// TEST 15 — مرحله‌ی ۲: سرورهای خروجی VLESS (exit/outbound)
//
...
/* تنظیماتِ قدیمی (بدون exits) باید هنوز بارگذاری شود */
...
for (const p of ['/api/exits']) {
  eq('auth: GET ' + p + ' guarded', (await call('GET', p, null, null)).status, 401);
}
for (const p of ['/api/exits', '/api/exits/default', '/api/exits/test']) {
