...
`concurrency` | ۶۴ | تعداد پروب‌های موازی (بالاتر = سریع‌تر) |
`timeout` | ۱۰۰۰ ms | تایم‌اوت هر پروب |
`probes` | ۲ | تعداد پروب برای تأییدِ هر آی‌پی |
...
`ports` | خالی | پورت‌های اسکن — خالی = پورت‌های TLS خودِ کانفیگ‌های کاربر |
...
### چطور کار می‌کند
- **پروب:** `fetch` روی `https://<ip>:<port>/cdn-cgi/trace` — چون گواهیِ آی‌پی معتبر نیست، *خطای سریع* یعنی لبه‌ی زنده جواب داده؛ *تایم‌اوت* یعنی مرده. پورت‌های کاندید موازی آزموده می‌شوند تا یک آی‌پیِ مرده فقط یک‌بار هزینه‌ی تایم‌اوت بدهد.
- **پوشش:** در حالت `even` هر رنج به‌نوبت سهم می‌گیرد (۲۰۴۸ نمونه ÷ ۱۵ رنج ≈ ۱۳۶ نمونه برای هر رنج) تا رنج‌های کوچک هم واقعاً اسکن شوند.
...
> برای تست موتور اسکنر بدون مرورگر: `node tests-manual/scanner-engine.cjs`
...
**اسکنر** | رادار آی‌پی تمیز سمت مرورگر • ۲۰۴۸ آی‌پی در هر اسکن • تمامِ رنج‌های رسمی کلودفلر • پروبِ موازی • ذخیره‌ی خودکار روی کانفیگ کاربر • تنظیمات کامل از پنل
