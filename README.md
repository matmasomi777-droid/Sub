...
> **نام پروژه:** Panel Core • **نسخه:** 4.0.0 • **زبان:** JavaScript (Workers) • **رابط:** فارسی/RTL
...
```bash
sh tests/run-all.sh          # نیازمند Node 22.5+ (node:sqlite)
```

تست‌ها `worker.js` را در محیطی شبیه‌سازی‌شده اجرا می‌کنند: پایگاه‌داده‌ی D1 یک SQLite واقعی است و `cloudflare:sockets`، KV و شیءِ ماندگارِ محدودکننده با بدل‌های سبک جایگزین شده‌اند. پوشش: شمارش مصرف، سقفِ آی‌پی (مدل Nova-Proxy)، و تستِ نشت برای مسیرهای آزادسازی / پاک‌سازی / رد.
