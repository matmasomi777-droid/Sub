...
### چطور کار می‌کند
- **پروب (روشِ اثبات‌شده‌ی پنل نوا):** یک `Image` روی `https://<ip>:<port>/cdn-cgi/trace`. لبه‌ی زنده‌ی کلودفلر گواهیِ نامطابق می‌دهد، پس مرورگر دستِ‌کم TLS را کامل می‌کند و بعد تصویر را رد می‌کند → `onerror` سریع. یعنی:
  - `onload` **یا** `onerror` = لبه پاسخ داد = **سالم**
  - **تایم‌اوت** = هیچ پاسخی نیامد = **مرده**

  چرا `Image` و نه `fetch`: پروبِ `fetch` به آی‌پیِ خام به `mode`/CORS/کش و `AbortController` گره خورده و روی شبکه‌ی ایران نتیجه‌ی ناپایدار می‌دهد. `Image` هیچ‌کدام از این وابستگی‌ها را ندارد.
- **پورت‌ها موازی** آزموده می‌شوند تا یک آی‌پیِ مرده فقط **یک** تایم‌اوت هزینه بدهد؛ پروب‌های تکمیلی فقط برای پورتِ برنده اجرا می‌شوند (میانگین، لرزش، افت، امتیاز = `avg + jitter×0.5 + loss×20`).
...
این برای هدفِ ما کافی است (رنج‌ها از پیش فقط کلودفلر هستند)، ولی اگر اعتبارسنجیِ قطعی می‌خواهید، [SenPai Scanner](https://github.com/MatinSenPai/SenPaiScanner) همین کار را بومی (با Xray و خواندن `colo`) انجام می‌دهد. در مرورگر عمداً از `fetch` استفاده نمی‌کنیم چون روی شبکه‌ی ایران نتیجه‌ی ناپایدار می‌دهد.
- **پوشش:** در حالت `even` هر رنج به‌نوبت سهم می‌گیرد (۲۰۴۸ نمونه ÷ ۱۵ رنج ≈ ۱۳۶ نمونه برای هر رنج) و **درونِ هر رنج آدرس تصادفی** انتخاب می‌شود — نه پشت‌سرهم از یک نقطه (که در بلوک‌های بزرگ مثل `104.16.0.0/13` به بازه‌های تخصیص‌نیافته می‌خورد و نتیجه را خالی می‌کرد).
...
> تست بدون مرورگر: `node tests-manual/scanner-engine.cjs` (موتورِ صفحه‌ی کاربر) و `node tests-manual/panel-scanner.cjs` (اسکنرِ داخلِ پنل)
...
**اسکنر** | رادار آی‌پی تمیز سمت مرورگر • ۲۰۴۸ آی‌پی در هر اسکن • تمامِ رنج‌های رسمی کلودفلر • پروبِ `Image` (روشِ نوا) • اجرای اسکن از خودِ پنل با جدولِ نتیجه • ذخیره‌ی خودکار روی کانفیگ کاربر • تنظیمات کامل از پنل
