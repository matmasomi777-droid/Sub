...
`minRtt` | ۰ | حداقل تأخیرِ قابل‌قبول — **۰ = بدون فیلتر** |
...
- **خودآزماییِ پروب:** پیش از هر اسکن، پروب روی سه آی‌پیِ آزمایشیِ RFC 5737 (`192.0.2.1` • `198.51.100.1` • `203.0.113.1`) اجرا می‌شود. این بازه‌ها هرگز مسیریابی نمی‌شوند، پس اگر پروب به آن‌ها «پاسخ» بدهد یعنی هر خطای سریعِ محلی دارد «زنده» تفسیر می‌شود و نتیجه بی‌اعتبار است — در آن حالت هشدار داده می‌شود و باید `minRtt` را روی ۶۰ بگذارید (همان راه‌حلِ نسخه‌ی اصلی).
