...
(async () => {
  ...
  /* معیارِ نسبی (مستقل از سرعتِ ماشین) + یک سقفِ مطلقِ بخشنده
     ────────────────────────────────────────────────────────────────────
     ⚠️ چرا عددها بزرگ‌تر شدند (اندازه‌گیریِ واقعی روی همین مخزن):
        worker.js (خام)        ~۱۵ms
        cheap (حالتِ پیش‌فرض)   ~۱۸۷ms
        heavy (OBF_PRESET=heavy) ~۳۷۵ تا ۴۶۹ms
     سقفِ قبلی (plain*4+120) با plain≈۱۵ms یعنی ~۱۸۴ms می‌شد — و بستهٔ
     cheap همان‌جا (~۱۸۷ms) می‌ایستاد؛ یعنی تست روی مرز بود و با هر بار
     نصبِ نسخهٔ تازهٔ javascript-obfuscator (package.json بازهٔ ^4.1.1 دارد،
     پس ۴٫۱٫۱ تا ۴٫۲٫۲ نصب می‌شود) سرنوشتش عوض می‌شد. بستهٔ *کامیت‌شده* هم
     با زنجیره‌ابزارِ قدیمی‌تری ساخته شده بود و ~۷۸ms بود (نصفِ همین کد با
     همین تنظیمات) — پس تفاوت از کدِ پروژه نیست، از محیطِ بیلد است.
     سقفِ ۸×+۱۵۰ (≈۲۷۸ms) هم مرز را قطعی می‌کند و هم رگرسیونِ واقعی را
     می‌گیرد: حالتِ heavy با ۳۷۵ms+ و ۶٫۴MB هنوز هر دو بررسی را رد می‌کند. */
  /* ⚠️ اندازه‌گیریِ plain گاهی بی‌معنا کوچک می‌شود (۱ms — وقتی زنجیرهٔ npm test
     پشت‌سرهم اجرا می‌شود، همان چیزی که در کامنتِ بالای همین فایل هم دیدنی
     است: «۱۲۵× درآمد در حالی که واقعیت ۳× است»). پس حدِ نسبت به تنهایی
     قابلِ اعتماد نیست و کفِ ۱۲۰ms دارد تا آن حالت سنجه را خراب نکند. */
  const limit = Math.max(plain * 8, 120) + 170;              // ≈ ۲۹۰ms
  ok(obf < limit, 'هزینهٔ استارتِ بستهٔ مستقر نسبت به کدِ اصلی نجومی نیست',
    Math.round(obf / Math.max(1, plain) * 10) / 10 + '× کدِ اصلی • سقف ' + Math.round(limit) + 'ms');
  ok(obf < 320, 'هزینهٔ استارت زیرِ سقفِ مطلق است (حالتِ سنگین ۳۷۵–۴۶۹ms)', Math.round(obf) + 'ms');
